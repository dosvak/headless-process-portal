...
## Public demo: https://portaldemo.dosvak.com (permanent)

* **Engine**: a dedicated IBM Business Automation Workflow 26.0.0.0 *Workflow Server* (Standard, offline, single cluster on WebSphere
  8.5.5.29 + Db2 11.5.8) in its own KVM virtual machine on the demo host (libvirt NAT network, 4 vCPU / 12 GB). The VM has **no
  outbound connectivity** and accepts connections **only from the demo host** (its hypervisor): the portal's nginx proxies `/rest/` and
  `/bpm/` to the VM's port 9443; consoles, Process Portal and every other port are not exposed. No Process Center connection: the
  sample application is delivered as an offline snapshot package (`BPMCreateOfflinePackage` on a Workflow Center,
  `BPMInstallOfflinePackage` on the demo engine).
* **Accounts**: `demo.manager`, `demo.finance`, `demo.ops`, `demo.support`, `demo.user` (password `demo1234`) exist only in the demo
  engine's file registry, are members of the sample application's teams and of no administrative group; the WebSphere / BAW
  administrators of the VM are not published. The login page lists the accounts (`environment.portaldemo.ts`, build configuration
  `portaldemo`).
* **Data**: seeded with `tools/seed_demo_data.py` against the demo engine; a nightly job on the host deletes the sample instances
  and re-seeds, so the demo stays tidy whatever visitors do.
* **Build / publish**: `npx ng build --configuration portaldemo` -> `dist/headless-portal/browser`, rsync to `/var/www/portaldemo.dosvak.com`
  (`DEPLOY_HOST=<host> DEPLOY_SITE=portaldemo.dosvak.com tools/deploy_demo.sh`).
* **TLS / DNS**: Cloudflare-proxied name (mode Full), origin certificate from the private lab CA on nginx; rate limit 20 req/s per client.


## First demo (2026-09-11): https://headless.quickpod.org - the lab Process Center behind a port mapping
