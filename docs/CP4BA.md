...
**Resolved on CP4BA 25.0.1 (TechZone, 2026-09-12):** the web designer's library offers no External Implementation (types available:
process, client-side / heritage human service, theme, view, localization resource, REST / web / Kafka / external / automation service,
AI agent, service flow, undercover agent, event subscription, team, business object, exposed process value, user attribute definition,
tracking group, web / server file) and there is no desktop designer on CP4BA. Coach tasks answer `url: null`, `clientTypes:
["IBM_WLE_Coach"]`, `externalActivityID: null` (`GET /task/{id}?includeURL=true`). The second contract source is therefore **not built**;
the `HeadlessForm` contract in the task data stays the only contract on both platforms.
...
4. Check the Studio library for *External Implementation* - done 2026-09-12 on 25.0.1: not offered (section 2.4), no second contract source.
